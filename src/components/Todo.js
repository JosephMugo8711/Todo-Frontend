...
class TodosContainer extends Component {
  ...
  updateTodoStatus = async (e, id) => {
    try {
      ...
      await axios.put(`/todos/${id}`, { todo: { completed: e.target.checked } });
  
      await this.saveTodosToServer(updatedTodos);
  ...
  saveTodosToServer = async (todos) => {
    try {
      ...
      console.error("Error saving todos to the server:", error);
    }
  };
                  ...
                  <div className="flex items-center mb-2">
                    ...
                    {this.state.editingTodoId === todo.id ? (
                      ...
                      <>
                        ...
                        <label className={`mb-2 ${todo.completed ? 'line-through' : ''}`}>
                              {todo.name}
                            </label>
                            <label className={`description-label ${todo.completed ? 'line-through' : ''}`}>
                              {todo.description}
                        </label>
